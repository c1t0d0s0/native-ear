      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto p-6 shadow-2xl space-y-6 text-slate-900 dark:text-white transition-colors">
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">学習データ・成績レポート</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="p-3.5 bg-slate-50 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700/60">
            ...
            <div className="text-2xl font-bold text-slate-900 dark:text-white">{totalAnswered} <span className="text-xs text-slate-500 dark:text-slate-400 font-normal">問</span></div>
          </div>
          ...
          <div className="p-3.5 bg-slate-50 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700/60">
            ...
            <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{stats.perfectCount} <span className="text-xs text-slate-500 dark:text-slate-400 font-normal">問</span></div>
          </div>
          ...
          <div className="p-3.5 bg-slate-50 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700/60">
            ...
            <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">{stats.streakDays} <span className="text-xs text-slate-500 dark:text-slate-400 font-normal">日</span></div>
          </div>
        </div>
                ...
                <div key={lvl} className="p-3.5 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/40 flex flex-col gap-2">
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-slate-600 dark:text-slate-400 font-medium">({info.subtitle})</span>
                    </div>
                    <div className="flex items-center gap-3 font-semibold text-slate-700 dark:text-slate-300">
                      <span>演習: {levelData.answered}問</span>
                      <span>正解: {levelData.perfect}問</span>
                      <span className="text-emerald-600 dark:text-emerald-400 font-bold">平均: {avg}%</span>
                    </div>
                  </div>
              ...
              {stats.history.slice(0, 10).map((h) => (
                <div key={h.id} className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-800/40 rounded-lg border border-slate-200 dark:border-slate-700/40 text-xs">
                  <div className="flex items-center gap-2 truncate mr-2">
                    ...
                    <span className="text-slate-800 dark:text-slate-300 truncate font-mono">{h.submittedText || '(未入力)'}</span>
                  </div>
