      ...
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-800 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-white tracking-tight">オープンソースライセンス (OSS Licenses)</h2>
              <p className="text-xs text-slate-400">本アプリケーションで使用しているオープンソースソフトウェアの著作権および許諾表示</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 overflow-y-auto space-y-3 flex-1">
          {OSS_LICENSES.map((item: LicenseItem, index: number) => {
            ...
            return (
              <div
                key={item.name}
                className="bg-slate-800/60 border border-slate-700/50 rounded-xl overflow-hidden transition hover:border-slate-600"
              >
                {/* Item Summary Header */}
                <div
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="truncate">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] text-slate-400 font-mono">v{item.version}</span>
                      </div>
                      <div className="text-xs text-slate-400 truncate">{item.copyright}</div>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 pt-2 border-t border-slate-700/40 space-y-2.5 bg-slate-900/40">
                    {item.repository && (
                      <div className="flex items-center justify-between text-xs pt-1">
                        <span className="text-slate-400">リポジトリ:</span>
                        <a
                          href={item.repository}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-1 text-indigo-400 hover:text-indigo-300 font-mono text-[11px] hover:underline"
                        >
                    ...
                    <div className="p-3 bg-slate-950/80 rounded-lg border border-slate-800 text-[11px] font-mono text-slate-300 whitespace-pre-wrap leading-relaxed max-h-48 overflow-y-auto">
                      {item.licenseText}
                    </div>
                  </div>
                )}
