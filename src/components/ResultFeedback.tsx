...
export const ResultFeedback: React.FC<ResultFeedbackProps> = ({
  ...
  return (
    <div className="w-full bg-white dark:bg-slate-800/90 border border-slate-200/90 dark:border-slate-700/80 rounded-2xl p-3.5 sm:p-5 shadow-lg dark:shadow-2xl space-y-3.5 animate-in fade-in slide-in-from-bottom-2 duration-300 transition-colors">
      {/* Header: Score & Badge */}
      <div className="flex flex-wrap items-center justify-between gap-2.5 pb-2.5 border-b border-slate-200 dark:border-slate-700/60">
        <div className="flex items-center gap-2.5">
          <div className="flex items-baseline gap-1 bg-slate-100 dark:bg-slate-900/80 px-3 py-1 rounded-xl border border-slate-200 dark:border-slate-700/60">
            <span className="text-xs text-slate-600 dark:text-slate-400 font-semibold">{t.resultFeedback.accuracy}</span>
            <span className={`text-lg font-black ${result.score >= 90 ? 'text-emerald-600 dark:text-emerald-400' : result.score >= 60 ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400'}`}>
              {result.score}%
            </span>
          </div>
          <span className={`text-xs font-bold px-2.5 py-1 rounded-xl border ${badge.color}`}>
            {badge.text}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onNext}
            className="flex items-center gap-2 px-4 py-1.5 rounded-xl text-xs font-bold bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white shadow-md shadow-indigo-500/20 active:scale-95 transition"
          >
      ...
      <div className="space-y-2">
        ...
        <div className="p-3 bg-slate-50 dark:bg-slate-900/80 rounded-xl border border-slate-200 dark:border-slate-700/80 flex flex-wrap gap-1.5 text-sm sm:text-base leading-relaxed font-mono">
          {result.tokens.map((token, index) => {
            if (token.type === 'correct') {
              return (
                <span
                  key={index}
                  className="px-1.5 py-0.5 rounded-md bg-emerald-100 dark:bg-emerald-500/20 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-500/30 font-bold"
                >
                  {token.text}
                </span>
              );
              ...
              return (
                <span
                  key={index}
                  className="px-1.5 py-0.5 rounded-md bg-rose-100 dark:bg-rose-500/20 text-rose-800 dark:text-rose-300 border border-rose-300 dark:border-rose-500/40 line-through decoration-rose-500 font-bold flex items-center gap-1"
              ...
              return (
                <span
                  key={index}
                  className="px-1.5 py-0.5 rounded-md bg-amber-100 dark:bg-amber-500/20 text-amber-800 dark:text-amber-300 border border-dashed border-amber-400 dark:border-amber-500/50 font-bold"
                >
                  [{token.expected}]
                </span>
              );
              ...
              return (
                <span
                  key={index}
                  className="px-1.5 py-0.5 rounded-md bg-slate-200 dark:bg-slate-700/60 text-slate-700 dark:text-slate-400 line-through border border-slate-300 dark:border-slate-600 font-medium"
                >
                  {token.actual}
                </span>
              );
      ...
      <div className="p-3 sm:p-3.5 bg-indigo-50/80 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-500/20 rounded-xl space-y-1.5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="flex items-center gap-1 px-2 py-0.5 bg-white dark:bg-indigo-600/30 hover:bg-indigo-50 dark:hover:bg-indigo-600/50 border border-indigo-200 dark:border-indigo-500/30 rounded-lg text-xs font-semibold text-indigo-700 dark:text-indigo-200 transition shadow-sm dark:shadow-none"
              title="1.0x"
            >
        ...
        <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 pt-1 border-t border-indigo-200/60 dark:border-slate-700/40">
          {sentence.japanese}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
        ...
        {sentence.vocabNotes && sentence.vocabNotes.length > 0 && (
          <div className="p-3 bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-200 dark:border-slate-700/60 space-y-1.5">
        ...
        <div className="p-3 bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-200 dark:border-slate-700/60 space-y-1.5">
          ...
          {sentence.linkingTip && (
            <p className="text-xs text-indigo-900 dark:text-indigo-300 leading-relaxed bg-indigo-50 dark:bg-indigo-500/10 p-1.5 rounded-lg border border-indigo-200 dark:border-indigo-500/20 font-medium">
