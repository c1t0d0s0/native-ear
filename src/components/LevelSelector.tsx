      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 sm:gap-2.5">
        {LEVELS.map((lvl) => {
          ...
          return (
            ...
            >
              {isSelected && (
                <span className="absolute -top-1.5 -right-1.5 w-3 h-3 bg-indigo-600 dark:bg-indigo-500 rounded-full ring-2 ring-white dark:ring-slate-900" />
              )}
              <div className="flex items-center justify-between mb-1">
                <span className={`text-base font-extrabold tracking-tight ${info.color}`}>
                  {lvl}
                </span>
                <span className="text-[10px] text-slate-700 dark:text-slate-300 font-semibold bg-slate-100 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700/40 px-1.5 py-0.5 rounded">
                  {info.target}
                </span>
              </div>
              ...
              <div className="mt-2 flex items-center justify-between text-[11px] text-slate-600 dark:text-slate-400 border-t border-slate-200 dark:border-slate-700/40 pt-1.5">
